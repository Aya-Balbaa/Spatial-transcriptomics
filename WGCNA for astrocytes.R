...
sft = pickSoftThreshold(trbg, powerVector = powers, networktype= "signed", verbose = 5)
# Plot the results:
sizeGrWindow(9, 5)
par(mfrow = c(1,2));
cex1 = 0.9;
...
net = blockwiseModules(trbg, power = 12,
...
nGenes = ncol(trbg);
nSamples = nrow(trbg);
MEs0 = moduleEigengenes(trbg, ModuleColors2)$eigengenes
MEs= orderMEs(MEs0)

NLF= factor(c(rep("-Away",6 ), rep("-Peri-Plaque", 6), rep("Trem2-Away", 4), rep("Trem2-Peri-plaque", 4)))
design3=model.matrix(~0+NLF)

moduleTraitCor = cor(MEs, design3, use = "p");
write.csv(moduleTraitCor, "GFAP-batch-15k-filtered-modultrait.csv")
